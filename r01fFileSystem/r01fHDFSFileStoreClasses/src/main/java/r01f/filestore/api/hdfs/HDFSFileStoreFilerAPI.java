...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	@Override
	public boolean hasSubfolder(final r01f.types.Path path) throws IOException {
		...
		for (int retryCount = 0; retry && retryCount < AUTH_CREDENTIAL_REFRESH_RETRY_NUM; retryCount++) {
			try {
				// ------------------------------------------------------------
				FileProperties[] folders = listFolderContents(path,
															   new FileFilter() {
																		@Override
																		...
																		@Override
																		public boolean accept(final r01f.types.Path path) {
																			return path.isFolderPath();
																		}
														   	   },
															   false);
				hasSubfolder = folders.length > 0;
				// ------------------------------------------------------------
				retry = false;
			} catch (IOException ioEx) {
				retry = _isCredentialExpiredError(ioEx);
			...
			try {
				...
				if (recursive) {
					...
					if (CollectionUtils.hasData(statusFiles)) {
						out = Stream.of(statusFiles)
								  	.map(HDFSFileProperties::fromOrNull)
								  	.filter(Objects::nonNull)
								  	.toArray(size -> new FileProperties[size]);
					} else {
